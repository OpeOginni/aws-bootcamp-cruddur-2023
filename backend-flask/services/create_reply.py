...
class CreateReply:
    def run(message, cognito_user_id, activity_uuid):
        ...
        if model['errors']:
            ...
            reply_count = CreateReply.update_reply_count(activity_uuid)
...
# Function to increase the reply count when a user replies
    def update_reply_count(activity_uuid):
        sql = db.template('activities', 'reply_increase')
        reply_count = db.query_commit(sql, {
